...
import TransferFRomComissionToPrimaryTable from "./components/table";
import { useRef, useState } from "react";
import TransferModal from "./components/TransferModal";
import { useQuery } from "@tanstack/react-query";
import { useSelector } from "react-redux";
import { getTransferCommissionToPrimaryLog } from "../../../services/transfer_comission_to_primary_service.js";
import { handleSetSearch } from "../../../utils/searchDebouncing.js";

function TransferFromComissionToPrimary() {
  const searchRef = useRef(null);
  const userInfo = useSelector((state) => state.auth.user);
  const [isTransferModalOpen, setIsTransferModalOpen] = useState(false);
  const [refetch, setRefetch] = useState(false);
  const [filter, setFilter] = useState({ search: "", limit: 10, page: 1 });
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["transferCommissionToPrimaryLogs", filter, refetch],
    queryFn: () => getTransferCommissionToPrimaryLog(),
  });
  ...
  return (
    <div className="w-full px-5 py-4 min-h-screen h-auto bg-[linear-gradient(135deg,_#EFF2FF_0%,_#FAF5FF_50%,_#FCF3FB_100%)] dark:bg-[linear-gradient(135deg,_#1e293b_0%,_#334155_50%,_#0f172a_100%)]">
      ...
      {/* table section */}
      <div className="mt-5">
        <TransferFRomComissionToPrimaryTable
          data={data?.data || []}
          isLoading={isLoading}
          isError={isError}
...
export default TransferFromComissionToPrimary;
