...
export default function TransferFRomComissionToPrimaryTable({
  data = [],
  isLoading = false,
  isError = false,
  onRetry = () => {},
        ...
        <Table>
          <TableHeader className="bg-gray-50 dark:bg-white/[0.02]">
            <TableRow>
              {[
                "Amount",
                "Commission Wallet Opening Balance",
                "Commission Wallet Closing Balance",
                "Primary Wallet Opening Balance",
                "Primary Wallet Closing Balance",
                "Created At",
              ].map((label) => (
                <TableCell
                  key={label}
            ...
            {isLoading
              ...
              : data.map((item, idx) => (
                  ...
                  >
                    ...
                    <TableCell className="px-5 py-3 text-sm text-gray-700 dark:text-gray-300">
                      {item?.commission_balance_before}
                    </TableCell>
                    <TableCell className="px-5 py-3 text-sm text-gray-700 dark:text-gray-300">
                      {item?.commission_balance_after}
                    </TableCell>
                    <TableCell className="px-5 py-3 text-sm text-gray-700 dark:text-gray-300">
                      {item?.main_wallet_balance_before}
                    </TableCell>
                    <TableCell
                      className={`px-5 py-3 text-sm text-gray-700 dark:text-gray-300`}
                    >
                      {item?.main_wallet_balance_after}
                    </TableCell>
                    <TableCell className="px-5 py-3 text-sm text-gray-700 dark:text-gray-300">
